...
def meal_detail(request, slug):
    filters = {
        ...
        "lunch": {"type": "Lunch"},
        "snacks": {"type": "Evening Snack"},
        "dinner": {"type": "Dinner"},
        "veg": {"category": "Veg"},
        "non-veg": {"category": "Non-Veg"},
        "10-min": {"timing__lte": 10},
        "indian": {"cuisine__iexact": "Indian cuisine"},
        "american": {"cuisine__iexact": "American cuisine"},
        "italian": {"cuisine__iexact": "Italian cuisine"},
    }

    if slug == "new":
        recipes = post.objects.filter(date_post__gte=timezone.now() - timedelta(days=7))

    else:
        filter_kwargs = filters.get(slug)

        if not filter_kwargs:
            recipes = post.objects.none()
        else:
            recipes = post.objects.filter(**filter_kwargs)

    recipes = recipes.select_related("author").prefetch_related("photo_set").order_by("-hit_count_generic__hits", "-date_post")
    ...
    return render(request, "yummyrecipes/meal_details.html", context)
...
def cuisines(request, slug):
    try:
        ...
        if slug == "Indian Cuisine":
            recipes = post.objects.filter(cuisine="Indian cuisine").order_by("-date_post")

        elif slug == "American Cuisine":
            recipes = post.objects.filter(cuisine="American cuisine").order_by("-date_post")

        elif slug == "Italian Cuisine":
            recipes = post.objects.filter(cuisine="Italian cuisine").order_by("-date_post")

        return render(
            request,
            "yummyrecipes/cuisines.html",
            {
                "slug": slug,
                "recipes": recipes,
            },
        ...
        return render(request, "yummyrecipes/cuisines.html", {"error_message": "An error occurred"})
...
def filtered_queryset(filter_name):
    ...
    return qs.filter(Q(type__iexact=query) | Q(category__iexact=query) | Q(cuisine__iexact=query))


def explore_filter(request):
    recipe_type = request.GET.get("type", "All")
    offset = int(request.GET.get("offset", 0))

    qs = filtered_queryset(recipe_type)

    posts = qs[offset : offset + PAGE_SIZE]

    return render(
        request,
        "yummyrecipes/partials/explore_cards.html",
        {
            "posts": posts,
            "recipe_type": recipe_type,
            "next_offset": offset + PAGE_SIZE,
            "has_more": qs.count() > offset + PAGE_SIZE,
            "historys_pk": get_historys_pk(request.user) if request.user.is_authenticated else [],
            "favourite_ids": set(request.user.favourite.values_list("id", flat=True)) if request.user.is_authenticated else set(),
        },
    )
...
class exploreRecipesView(HitCountMixin, ListView):
    model = post
    template_name = "yummyrecipes/explore.html"
    ...
    def get_context_data(self, **kwargs):
        ...
        context["historys_pk"] = get_historys_pk(user) if user.is_authenticated else []

        context["favourite_ids"] = set(user.favourite.values_list("id", flat=True)) if user.is_authenticated else set()
...
class postDetailView(HitCountDetailView):
    model = post
    template_name = "yummyrecipes/postdetail.html"
...
class trendingRecipesView(ListView):
    model = post
    template_name = "yummyrecipes/trending.html"
    context_object_name = "posts"
    ...
    def get_base_queryset(self):
        ...
        if cached:
            ...
            ordered = sorted(qs, key=lambda x: ids.index(x.id))
            return ordered
    ...
    def get_queryset(self):
        return self.get_base_queryset()[: self.PAGE_SIZE]
    ...
    def get_context_data(self, **kwargs):
        ...
        context["next_offset"] = self.PAGE_SIZE
        context["has_more"] = len(self.get_base_queryset()) > self.PAGE_SIZE
    ...
    def render_to_response(self, context, **response_kwargs):
        if self.request.headers.get("HX-Request"):
            return render(
                self.request,
                "yummyrecipes/partials/trending_cards.html",
                context,
            )
        return super().render_to_response(context, **response_kwargs)


def load_more_trending_recipes(request):
    PAGE_SIZE = 8
    offset = int(request.GET.get("offset", 0))

    view = trendingRecipesView()
    view.request = request

    posts = view.get_base_queryset()
    ...
    return render(
        ...
        "yummyrecipes/partials/trending_cards.html",
        {
            ...
            "historys_pk": get_historys_pk(request.user) if request.user.is_authenticated else [],
            "favourite_ids": set(request.user.favourite.values_list("id", flat=True)) if request.user.is_authenticated else set(),
            "has_more": offset + PAGE_SIZE < len(posts),
            "next_offset": offset + PAGE_SIZE,
        },
    )
...
def refresh_recipes(request):
    ...
    html = render_to_string(
        "yummyrecipes/recipes_section.html",
        {
